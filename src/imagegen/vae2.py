...
class Decoder(nn.Module):
    def __init__(self, decoder_net, distribution="categorical", num_values=256):
        ...
        assert distribution in [
            "categorical",
            "bernoulli",
        ], "Distribution should be categorical or bernoulli"
...
class VAE(nn.Module):
    def __init__(
        ...
        self.decoder = Decoder(
            decoder_net=decoder_net, distribution=likelihood_type, num_values=num_values
        )
    ...
    def forward(self, x, reduction="avg"):
        ...
        KL = self.prior.log_prob(z) - self.encoder.log_prob(
...
def get_encoder_decoder(
    dim_X=218 * 178 * 3, dim_H=256, dim_Z=20, num_pixel_values=256, device="mps"
):
